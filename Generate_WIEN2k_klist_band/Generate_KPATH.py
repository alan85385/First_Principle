...
def main():
    ...
    kpoints_matrix, lattice_type, kpoints = get_parameter(library) #Get parameter from user
    kpath_denominators, number = transform_kpoints_matrix_into_kpath(kpoints_matrix) #Transform kpoints into kpath
    write_file(kpath_denominators, lattice_type, kpoints, number) #Write two files: klist_band and klabel
...
def get_parameter(library):
    ...
    '''
    To get parameters which include lattice type and kpoints from user
    '''

    file_name = 'kpoints.txt' # A txt file contains kpoints

    if len(library) == 0: # Automatically activate matrix mode if there is no library
        ...
        while True:
            ...
            if lattice_type == 'm': # Activate matrix mode
                kpoints_matrix, kpoints = matrix_mode(file_name)
                break

            elif lattice_type in library.keys(): # Activate string mode
...
def matrix_mode(file_name):
    ...
    '''
    The function matrix mode can read kpoints.txt or enter kpoints by user
    '''

    try:
        kpoints_matrix, kpoints = read_kpoints(file_name) # Read kpoints.txt
        print('Successfully read the file %s' %file_name)

    except:
        print('Not find the file %s' %file_name)
        kpoints_matrix, kpoints = ask_kpoints_vector() # Enter kpoints by user
...
def read_kpoints(file_name):
    ...
    '''
    The function read kpoints can read kpoints from kpoints.txt and transform kpoints into kpoints matrix
    '''
    ...
    for line in lines:
        ...
        if len(line) == 3:
            ...
            try:
                kpoints_matrix.append(np.array(line, dtype = 'float')) # Create kpoints matrix
                kpoints.append(line)
            except:
                sys.stderr.write('There exists some non-numeric type data of k vectors in the file %s\n' %file_name)
...
def ask_kpoints_vector():
    ...
    '''
    The function ask kpoints vector can collect kpoint with vector form from user
    '''
    ...
    while True:
        ...
        if kpoint[0] == 'd' and len(kpoint) == 1: # Complete collecting
            break
        elif len(kpoint) == 3: # Collect kpoint
            try:
...
def ask_kpoints_string(kpoints_library):
    ...
    '''
    The function ask kpoints string can collect kpoint with string form from user
    '''
...
def write_file(kpath_denominators, lattice_type, kpoints, number):
    ...
    write_KPOINTS(filename, kpath_denominators)
...
def write_KPOINTS(filename, kpath_denominators):
